...
So we are going to learn how to control the arduino.  You did this by dragging blocks onto the screen.  Each block represents something that the arduino will do.  The first thing we are going to learn how to do is send electricity to the pin.
...
10) Click the upload button.  This will upload program to the arduino.  Once you are done you should see a yellow light.
...
11) Take the bread board and plug the long end of the led and put into (3, e) of the breadboard and the long end into (5, e).
...
14) Take a male to male jumper wire and plug into (gnd) ground of the arduino and the end space of the blue line on the breadboard.
...
16) (Challenge) Can u change the program to make led light up by sending electricity to pin 7.  To see your change click the upload button on the block web page.  If you have trouble ask for help. :)
...
![step17](/images/arduino-block/2017-10-12-input-output/step17.jpg)
 ...
 Now it's time to make to Led blink.  Do that we are going to learn a new block called delay.  Delay pauses the program for period of time.  The number you put in delay determines how many milliseconds the program will pause.  A millisecond is 1/1000 of a second.
...
So the DigitalWrite Block takes 2 inputs, the pin number and where to send electricity to the pin.  If it is set to HIGH it will send electricity to the pin.  If it is set to LOW it will not send electricity to the pin.
...
22) (Challenge) Notice that led does not blink. Can you fix the blocks to make the led blink.  This is called debugging.  Debugging is when something goes wrong with program and you are tasked with fixing it.  A bug is what is wrong with the program. 
...
26) Now drag DigitalRead with PULLUP RESISTOR block to screen.  Set the pin number 2.
...
27) Drag the Serial Print Block onto the screen.  This block is used to send messages to our computer from the arduino.
...
29) Connect the delay block to the bottom of the serial block.
...
32) Go to tools -> serial monitor.  Notice that it is output 1.  If you press the button it will start to output 0.  So when the button is pressed the block reads 0.  Once you are done with the step be sure and close the serial monitor.
...
Serial Block is a great way to debug. :)

33) Now we are going to learn about variables.  Variables is how we store data in the arduino.  Go ahead and delete all the blocks and pull out variable block.
...
Now we are going to learn about if blocks.  If blocks are used to make decisions in your program.  If what is connected to the if block is true it will do all the blocks in the do blocks.  Let's build something to make this concept more clear.
...
37) Now drag the compare block onto the screen and connect it to the if block.  So if what is on the right and left side equal each other what is inside the do block will be done.  
...
39) Now go the math section and drag the number block the second part of the compare block.
...
40) Drag Digital Write Block into the do.  Set the pin to 8.
...
41) Upload program to the arduino and push the button.  Notice that when u stop pressing the button the light continues to stay one.  Can you tell me why?
...
- Make it so that when u push the button the led blinks 2 times.

- Ask for a second led and wire set.  Hook the other led to the side of the led.  Make so that when u push the button it makes the right led on and the left led off.  And when you don't push the button make so the opposite happens. 
